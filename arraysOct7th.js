...
//end of people index array called data
//Note, each position in data has an object with information on each person

//within the person object, named as a position in data,
//the address key has an object that contains addresses

//within address, the geo key contains an object of lat and lng

//outside of address key but still within person object,
//the company key contains an object with company information.


//1. find out how many people have .biz email addresses and print their names

var bizArray = data.filter(function(person){
  if(person.email.indexOf(".biz") >= 0 ){
    console.log(person.name);
    return person;
  }
  //only grab a person if they have a .biz string present in their email string value.
  //console.log their name
  //return the person object into the new bizArray
});

console.log(bizArray.length + " people have .biz email addresses");



//2. Extract the name of the city where the .biz email addresses live

var cityNames = bizArray.map(function(person){
...
//3. Keep only the unique cities on the list, don't use filter
var uniqueCities = cityNames.reduce(function(double, city){
...
//print remaining data on the screen

var printData = uniqueCities.forEach(function(city){
  console.log(city);
});
...
//1. Find out if it will rain tody using the hourly data, should use a method that returns a boolean.

var rainy = weatherData.hourly.data.some(function(dataObj){
  var string = dataObj.summary;
  if(string.indexOf("ain") >= 0){
    return true;
  }
  
});
//use the some method!
//only a few array methods return boolean values
//every only returns true if every element of an array passes the test whereas
...
//this is just a quick function finding the hours of our weather data
var time = weatherData.hourly.data.map(function(element){
  var unix = new Date(element.time * 1000);
  //we need to multiple our element.time value by 1000 in order to convert seconds into milliseconds
  //JS date methods require unix time stamps with millisecond values
  
  return unix.getHours();
  //this returns an array of every hour of the day for 48 hours. The hours are written in military time
});




//2. Print which hours it will rain within the span of eight hours from the current hour

var currentHour = new Date().getHours();
//the current hour of the day


var rainyDay = weatherData.hourly.data.filter(function(dataObj){
  ...
  return new Date(rainy.time * 1000).getHours();
  //this map method returns all of the hours that it is raining
  //we multiply 1000 so that our unix amount include milliseconds
  //JS date methods require milliseconds
})


var extraHours = rainyDay.indexOf(currentHour);
rainyDay = rainyDay.splice(extraHours, rainyDay.length);
//this little tidbit at the end is to trim the excess hours
//to only include 24hrs rather than 48hrs.

var eightHourForecast = rainyDay.filter(function(hour, idx, array){

   if(currentHour > 15 && hour < 15){
   var twelveHours = currentHour - 15;
     if(twelveHours >= hour){
       return hour;
   }
 }
